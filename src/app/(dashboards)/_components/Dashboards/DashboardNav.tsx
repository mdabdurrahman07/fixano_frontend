      ...
      <aside className="hidden md:flex flex-col h-screen w-64 fixed left-0 top-0 bg-[#2d3133] border-r border-[#bccac0] shadow-md z-40 py-8">
        <div className="px-6 mb-10 mt-16">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <div className="font-['Inter'] font-semibold text-[14px] text-white">{user.name}</div>
              <div className="font-['Inter'] font-medium text-[12px] text-[#bccac0]">{user.role}</div>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 flex flex-col gap-2 px-4">
          {routes.map((route) => {
            ...
            return (
              ...
              >
                <Icon size={20} />
                <span className="font-['Inter'] font-medium text-[14px]">{route.name}</span>
              </Link>
            );
          })}
        </nav>
        ...
        <div className="px-6 mt-auto">
          <button className="w-full h-12 bg-[#006948]/20 border border-[#006948]/30 hover:bg-[#006948]/40 transition-colors rounded-xl flex items-center justify-center gap-2 text-[#85f8c4] font-['Inter'] text-[14px] font-semibold">
            <LogOut size={16} />
            Sign Out
          </button>
        </div>
      ...
      <nav className="fixed bottom-0 left-0 w-full flex justify-around items-center px-4 pb-4 pt-2 bg-[#f7f9fb]/80 backdrop-blur-xl border-t border-white/20 shadow-lg md:hidden z-50">
        {routes.slice(0, 4).map((route) => {
          ...
          return (
            ...
            >
              <Icon size={24} />
              <span className="font-['Inter'] font-medium text-[12px] mt-1">{route.name}</span>
            </Link>
          );
        })}
      </nav>
